...
from .base import (
    SerialDriver, SerialPortInfo, DriverType, 
    SerialConnectionError, DriverNotAvailableError, SerialDriverError
)
...
class CP2102Driver(SerialDriver):
    ...
    def __init__(self):
        ...
        self.pid_pattern = [0xEA60, 0xEA61, 0xEA70, 0xEA71, 0xEA63, 0xEA64, 0xEA65, 0xEA66]
    ...
    def _is_cp2102_device(self, port) -> bool:
        ...
        return any(keyword in port_info for keyword in ['cp2102', 'cp2103', 'cp2104', 'cp2105', 'cp2108', 'cp2109', 'silicon labs', 'uart bridge'])
    ...
    async def connect(self, port_info: SerialPortInfo, **kwargs) -> serial.Serial:
        ...
        try:
            ...
            raise SerialConnectionError(f"Failed to connect to CP2102 {port_info.device}: {str(e)}") from e
