...
class DriverManager:
    ...
    def detect_all_devices(self) -> List[SerialPortInfo]:
        ...
        for driver_type, driver in self.drivers.items():
            try:
                ...
                self.logger.error(f"Error scanning with {driver_type.value} driver: {e}")
        ...
        if port_info.vid and port_info.pid:
            for driver_type, driver in self.drivers.items():
                if self._device_matches_driver(port_info, driver):
                    self.logger.info(f"Matched {port_info.device} to {driver_type.value} driver")
                    return driver
        
        # Fallback to generic handling
        return self.drivers.get(DriverType.GENERIC)
    
    def _device_matches_driver(self, port_info: SerialPortInfo, driver: SerialDriver) -> bool:
        """
    ...
    def _remove_duplicate_devices(self, devices: List[SerialPortInfo]) -> List[SerialPortInfo]:
        """
