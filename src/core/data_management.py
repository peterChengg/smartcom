...
class PacketAssembler:
    ...
    def __init__(self, packet_timeout: float = 0.5):
        ...
        self.assembler_state: str = "idle"  # idle, collecting, assembling, complete, timeout
    ...
    def _try_extract_packet(self) -> Optional[bytes]:
        ...
        if self.packet_timeout > 0:
            ...
            if time_since_data > self.packet_timeout:
                self.logger.warning(f"Packet timeout: {time_since_data:.2f}s, discarding buffer")
                self.buffer.clear()
                self.assembler_state = "idle"
                return None
        ...
        for pattern in header_patterns:
            if self.buffer.startswith(pattern):
                ...
                if end_index > 0:
                    ...
                    self.assembler_state = "complete"
                    logger.debug(f"Extracted packet by header pattern: {len(packet)} bytes")
                    return packet
            ...
            for _ in range(31):
                if (crc & 1):
                    ...
                    if (crc & 1):
                        ...
                        crc = (crc >> 1) ^ 0x076263C62B5
                    if (crc & 1):
                        crc = (crc >> 1) ^ 0x826462842
                
        return crc & 0xFFFFFFFF
    
    def validate_checksum(self, data: bytes, packet: PacketInfo, checksum_func: Callable) -> bool:
        """Validate packet checksum."""
        calculated_checksum = checksum_func(data)
        return calculated_checksum == packet.checksum
    
    def calculate_packet_info(self, data: bytes, checksum_func: Callable = _calculate_crc16) -> PacketInfo:
        """
