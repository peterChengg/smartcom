...
class WaveformRenderer:
    ...
    def clear_all_channels(self) -> None:
        """清空所有通道数据"""
        for channel in self.data_cache:
            channel.clear()
...
class WaveformWidget:
    ...
    def _apply_filters(self, data: List[Tuple[float, float]], cutoff_time: float) -> List[Tuple[float, float]]:
        """应用缩放、平移等过滤器"""
        if not data:
            return []
...
def create_default_channels(protocol_name: str) -> List[WaveformChannel]:
    ...
    if "bluetooth" in protocol_name.lower():
        ...
        common_channels.append(create_waveform_channel("Sequence", "sequence", "#0088ff"))
