...
class DataBuffer:
    ...
    def add_data(self, packet: ParsedPacket) -> None:
        ...
        self.timestamps.append(packet.timestamp if hasattr(packet, 'timestamp') else time.time())
...
class ProtocolVisualizationEngine:
    ...
    def get_protocol_data(self, protocol_name: str, count: Optional[int] = None) -> List[ParsedPacket]:
        """获取指定协议的数据"""
        if protocol_name not in self.data_buffers:
            return []
        return self.data_buffers[protocol_name].get_data(count)
        
    def get_protocol_data_by_time(self, protocol_name: str, duration: float) -> List[ParsedPacket]:
        """获取指定协议时间范围内的数据"""
        if protocol_name not in self.data_buffers:
            return []
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        if stats["frames_rendered"] > 0:
            stats["avg_render_time"] = stats["total_render_time"] / stats["frames_rendered"]
        return stats
    ...
    def get_data_summary(self, protocol_name: str) -> Dict[str, Any]:
        ...
        for field_name in field_stats:
            field_stats[field_name]["unique_values"] = list(field_stats[field_name]["unique_values"])
...
# 工厂函数
def create_visualization_engine(config: Optional[VisualizationConfig] = None) -> ProtocolVisualizationEngine:
    """创建可视化引擎"""
    if config is None:
