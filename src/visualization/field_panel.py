...
class FieldFilter:
    ...
    def matches_filter(self, node: FieldTreeNode) -> bool:
        ...
        # 文本过滤
        if self.filter_text:
            if self.filter_text not in node.name.lower() and self.filter_text not in node.get_display_value().lower():
                return False
...
class FieldPanel:
    ...
    def __init__(self, width: int = 400, height: int = 600):
        ...
        # 统计信息
        self.field_stats: Dict[str, Any] = defaultdict(lambda: {
            "count": 0,
            "total_updates": 0,
            "last_update": 0
        })
    ...
    def _merge_field_tree(self, existing_root: FieldTreeNode, new_tree: FieldTreeNode) -> None:
        """合并字段树"""
        timestamp = time.time()
    ...
    def _process_node(self, node: FieldTreeNode, depth: int) -> Dict[str, Any]:
        ...
        if node.children:
            # 自动展开前几层
            is_expanded = depth < self.auto_expand_depth or node.name in self.expanded_nodes
    ...
    def _collect_paths_recursive(self, node: FieldTreeNode, current_path: str, path_set: set) -> None:
        """递归收集节点路径"""
    ...
    def _find_node_recursive(self, node: FieldTreeNode, path: str) -> Optional[FieldTreeNode]:
        """递归查找节点"""
        if not path:
            return node
    ...
    def export_field_structure(self, filename: str, protocol_name: Optional[str] = None) -> bool:
        """导出字段结构"""
        try:
            import json
            ...
            protocols_to_export = [protocol_name] if protocol_name else list(self.root_nodes.keys())
